...
const COLOR_HIGHLIGHT_BG = rgb(1, 0.96, 0.6)
...
/**
 * @param {string} raw
 * @returns {'blank' | 'section-header' | 'missing' | 'field-label' | 'highlight' | 'body'}
 */
...
export async function buildTranslationPdf(translated, binaries) {
  ...
  for (const rawPara of paragraphs) {
    ...
    for (let i = 0; i < wrapped.length; i++) {
      ...
      const drawKind =
        kind === 'section-header' && i > 0 ? 'header-continuation' : kind
